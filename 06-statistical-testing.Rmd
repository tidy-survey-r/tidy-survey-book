...
<!--<JM>Why no dot in front of "design" like in front of "data" in the mtcars example?-->
```r
svydata_des %>%
 svyttest(design = ., x ~ y)
```
...
The arguments are:
<!--<JM> Don't understand last bullet as written-->
...
<!--<JM> Add a link for the "beginning of this chapter" so reader can jump right there?-->
...
<!--<JM> Why would you want to test difference from 68? Providing motivation makes example more real and relatable.-->
...
- $H_A: \mu \neq 68$
<!--<JM> Next sentence is confusing as written-->
...
<!--<JM> Does changing the confidence interval change your conclusion? See the example through.-->
```{r}
...
<!--<JM> Does this test assume there's no variance around the Germany number?-->
...
```
<!--<JM> Used to seeing p-values as .0000.... Maybe convert it for the reader. Like, <.00001-->
...
Additionally, as with the t-test function, both `svygofchisq()` and `svychisq()` have the `na.rm` argument. If any data are missing, the $\chi^2$ tests will assume that `NA` is a category and include it in the calculation. Throughout this chapter, we will always set `na.rm = TRUE`, but before analyzing the survey data, review the notes provided in Chapter \@ref(c04-understanding-survey-data-documentation) to better understand how to handle missing data.
...
chi_ex1
```
<!--<JM> Superscripts not appearing correctly in book version-->
...
<!--<JM> Why no legend? It would be helpful to include.-->
```{r}
...
<!--<JM> Reorder columns by factor levels-->
```{r}
...
<!--<JM> Would be another good one to visualize vs. a table-->
```{r}
...
<!--<JM> Why is this answer put in code?-->  
```{r}
