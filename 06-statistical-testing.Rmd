...
Or, we can use the `survey_mean()` function described in Chapter \@ref(c05-descriptive-analysis): \index{Functions in srvyr!survey\_mean}
...
To conduct this in R, let's first look at the education variable (`Education`) we have on the ANES data. Using the `survey_mean()` function discussed in Chapter \@ref(c05-descriptive-analysis), we can see the education levels and estimated proportions. \index{Functions in srvyr!survey\_mean}
