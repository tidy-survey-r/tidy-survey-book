...
To create an interaction plot, the y-axis is the predicted probabilities, and one of our x-variables is on the x-axis and the other is represented by multiple lines. Figure \@ref(fig:model-logisticexamp-biden-plot) shows the interaction plot with gender on the x-axis and early voting behavior represented by the lines.
...
Interactions in models can be difficult to understand from the coefficients alone. Using these interaction plots can help others understand the nuances of the results, and often can become even more helpful with more than two levels in a given factor (e.g., education or race/ethnicity). \index{Functions in survey!svyglm|)}
