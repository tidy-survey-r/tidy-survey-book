...
These results can then be used in a variety of ways, including examining residual plots as illustrated in the code below and Figure \@ref(fig:model-aug-examp-plot). In the residual plot, we wish to see no patterns as it may indicate heteroscedasticity. In that case, the standard errors of the coefficients may be incorrect but this residual plot does not show a strong pattern.
...
Logistic regression is used to model a binary outcomes such as whether or not someone voted. There are several instances where an outcome may not be originally binary but is collapsed into being binary. For example, given that gender is not actually on a binary scale, many researchers now code gender in logistic modeling as cis-male compared to not cis-male. We could also convert a 5-point likert scale that has levels of "Always", "Most of the time", "About half the time", "Some of the time", and "Never" to group the first levels into one group as an outcome.
...
Let's look at another example with interaction effects.  If we're interested in understanding the demographics of people who voted for Biden among voters, we could include `Gender` and `Education` in our model.  

First we need to subset the data to voters and then create an indicator for voted for Biden. Note that this indicator places anyone who did not vote at all into `VoteBiden = 0`.
```{r}
...
This main effect model indicates that respondents with a graduate degree are `r signif(log_biden_main$coefficients[6],3)` (p-value=`r tidy(log_biden_main) %>% slice(6) %>% pull(p.value) %>% pretty_p_value()`) times more likely to vote for Biden compared to respondents with less than a high school degree. We see that gender is also significant (p-value=`r tidy(log_biden_main) %>% slice(2) %>% pull(p.value) %>% pretty_p_value()`). 
...
The results from the interaction model shownone of the interactions are significant.  To better understand what the interactions, we will want to plot the predicted probabilities.  Let's first obtain the predicted probabilities for each possible combination of variables using the `augment()` function.
