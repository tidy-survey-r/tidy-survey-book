...
To perform this type of analysis in R, the general syntax is as follows: \index{Functions in survey!svyglm}
...
In the following code, we test whether this temperature varies by region by first using `svyglm()` to run the test and then using `broom::tidy()` to display the output. Note that the temperature setting is set to NA when the household does not use air-conditioning, and since the default handling of NAs is `na.action=na.omit`, records that do not use air-conditioning will not be included in this regression. \index{Functions in survey!svyglm}
...
The syntax for this regression uses the same function as ANOVA, but can have more than one variable listed on the right-hand side of the formula: \index{Functions in survey!svyglm}
...
Given that the plot shows a potential increasing relationship between square footage and electricity expenditure, fitting a model will allow us to determine if the relationship is statistically significant.  The model is fit below with electricity expenditure as the outcome. \index{Functions in survey!svyglm}
...
The syntax for logistic regression is as follows: \index{Functions in survey!svyglm}
...
By looking at Figure \@ref(fig:model-logisticexamp-plot) it appears that people who voted for Trump are more likely to say that they usually have trust in the government compared to those who voted for Biden and Other candidates. To determine if this insight is accurate, we next we fit the model. \index{Functions in survey!svyglm}
...
Let's first look at the main effects of gender and early voting behavior. \index{Functions in survey!svyglm}
```{r}
...
It is possible that there is an interaction between gender and early voting behavior. To determine this we can create a model that includes the interaction effects: \index{Functions in survey!svyglm}
```{r}
