...
anes_des_der %>%
  ...
  summarise(pct_trust = survey_mean(TrustGovernmentUsually,
                                    na.rm = TRUE,
                                    proportion = TRUE,
                                    vartype = "ci"),
    .groups = "drop") %>%
...
First we need to create an indicator for voted for Biden:
```{r}
...
log_biden_pred<-log_biden_int %>%
  ...
  mutate(.se.fit = sqrt(attr(.fitted, "var")), 
         .fitted = as.numeric(.fitted)) %>%
  select(VoteBiden,Gender,Education,
         .fitted,.se.fit) 
```
...
#| fig.alt: !expr str_c(BrailleR::VI(biden_int_plot)$text, collapse=" ")

biden_int_plot<-log_biden_pred %>% 
  filter(VoteBiden==1) %>% 
  distinct() %>% 
  arrange(Gender,Education) %>% 
  ggplot(aes(x=Gender,y=.fitted,group=Education,color=Education,linetype=Education)) +
  geom_line() +
  geom_point() + 
