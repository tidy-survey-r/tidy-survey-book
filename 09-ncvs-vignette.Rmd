...
The data from ICPSR are distributed with five files with their unique identifiers indicated:
...
We will focus on the household, person, and incident files. From these files, we selected a subset of columns for examples and have included the subset on the book's GitHub repository, and the file subsets will be used in this vignette. Find the dataset in the book's GitHub repository or download the complete files at ICPSR^[https://www.icpsr.umich.edu/web/NACJD/studies/38429]. 
...
In the code above, some checking of derived variables is done. This is a good point to pause to look at the output of crosswalks between an original variable and a derived to check the logic was programmed correctly and everything ends up in the bucket you expect.  

After creating indicators of victimization types and characteristics, the file is summarized, and crimes are summed across persons or households by `YEARQ.` Property crimes are summed across households, and personal crimes (violent, purse snatching, and pocket-picking) are summed across persons. The indicators are summed using the `serieswgt`, and the variable `WGTVICCY` needs to be retained for later analysis.
...
A final step in file preparation for the household and person files is creating any derived variables on the household and person files, such as income categories or age categories, for subgroup analysis. This step can be done before or after merging the victimization counts.
