...
For victimization rates, we need to know the victimization status for both victims and non-victims. The incident file must be summarized and merged onto the household or person files for household-level and person-level crimes, respectively. We begin this vignette by discussing how to create these incident summary files. This is following Section 2.2 of the NCVS User's Guide (@ncvs_user_guide).
...
Table: Codebook for incident variables - related to series weight
...
Table: Codebook for incident variables - crime type indicators and characteristics
...
Table: Codebook for household variables
...
Table: Codebook for person variables
...
The number of victimizations estimated using the incident file is equivalent to the person and household file method.
...
As mentioned in the introduction, victimization rates use the incident weight the numerator and the person or household weight in the denominator. This is accomplished by calculating the rates with the weight adjustment (`ADJINC_WT`) multiplied by the estimate of interest. In the example below, this is property victimization.
...
In the chunk above, we see the estimate for property victimization rate in 2021 was `r formatC(vr_prop$Property_Rate, format="f", digits=1)` per 1,000 households which is consistent with calculating as the number of victimizations per 1,000 population as demonstrated in the next chunk:
