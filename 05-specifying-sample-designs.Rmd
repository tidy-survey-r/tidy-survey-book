...
This standard error estimate might look very similar to equations in other applications except for the part on the right side of the equation: $1-\frac{n}{N}$. This is call the finite population correction factor (FPC), and if the size of the frame, $N$, is very large, the FPC is negligible so it is often ignored. 
...
- Examples: 
  ...
  - Example 2: Consider a study sending out interviewers to households for a survey. This is a more complicated example that requires two levels of selection.  First, counties could be selected as the PSU, then Census block groups within counties selected as the secondary sampling unit (SSU).  Households are then randomly sampled within the block groups.  This type of design is popular for in-person survey as it reduces the travel necessary for interviewers.
...
If a dataset had WT for the main weight and had 20 BRR weights indicated REPWT1, REPWT2, ..., REPWT20, the following syntax could be used (both are valid):
