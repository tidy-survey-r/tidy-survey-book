...
The order of these steps matters in survey analysis. For example, if we need to subset the data, \index{Functions in srvyr!filter}we must use `filter()` on our data **after** creating the survey design. If we do this before the sampling design is created, we may not be correctly accounting for the sampling design, resulting in incorrect findings. 
