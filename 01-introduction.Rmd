...
This book focuses on how to **analyze** the data collected from a survey. We assume that you have conducted a survey or obtained a microdata file.  Microdata, also known as respondent-level or row-level data, contains individual survey responses, analysis weights, and design variables (as opposed to opposed to summarized data in tables). For the purposes of this book, you need the weights and design variables for your survey data. These are required to accurately calculate unbiased estimates^[Note, if you do not already have weights created for the survey data you are using, we recommend reviewing other resources focused on weight creation such as @Valliant2018weights to create weights before working on analysis.]. Understanding the concepts and techniques discussed in this book will help you to extract meaningful insights from your survey data.

To account for the weights and study design, researchers rely on statistical software such as SAS, Stata, SUDAAN, and R. In this book, we will use R to provide an overview to survey analysis. Our goal is to provide a comprehensive guide for individuals new to survey analysis but have some statistics and R programming background. We will use a combination of both the {survey} and {srvyr} packages and present the code following best practices from the tidyverse [@R-srvyr; @lumley2010complex; @tidyverse2019]. 

In 2003, the {survey} package was released on CRAN and has been continuously developed over time. This package, primarily developed by Thomas Lumley, is extensive and includes the following features:
...
The {srvyr} package in R builds on the {survey} package. It provides wrappers for functions that align with the tidyverse philosophy, which is our motivation for using and recommending this package. We find that the {srvyr} package is user-friendly for those familiar with tidyverse packages in R [@tidyverse2019]. For example, while many functions in the {survey} package use variables as formulas, the {srvyr} package uses tidy selection to pass variable names (a common feature in the tidyverse) [R-tidyselect]. Users of the tidyverse are most likely familiar with the magrittr pipe (`%>%`), which seamlessly works with functions from the {srvyr} package. Moreover, several common functions from {dplyr}, such as including `filter()`, `mutate()`, and `summarize()`, can be applied to survey objects [@R-dplyr]. Users can streamline their analysis workflow and capitalize on the benefits of both the {srvyr} and {tidyverse} packages.
...
```
To load the RECS and ANES datasets, start by running `library(srvyrexploR)` to load the package. The {srvyrexploR} uses lazy loading so it is not necessary to load the data explicitly into the environment. The datasets we will use are `recs_2020` and `anes_2020`.
