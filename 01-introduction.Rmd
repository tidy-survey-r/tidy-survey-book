...
The {srvyr} package in R builds on the {survey} package. It provides wrappers for functions that align with the tidyverse philosophy, which is our motivation for using and recommending this package. We believe the {srvyr} package is easy to learn for R users who already use {tidyverse} packages. For example, variables to many functions in the {survey} package are passed as formulas, but in the {srvyr} package, variable names are passed using tidy select^[https://dplyr.tidyverse.org/reference/dplyr_tidy_select.html] (a common feature in the tidyverse). Users of the tidyverse are most likely familiar with the magittr pipe (`%>%`) which will seamlessly work with functions from the {srvyr} package. Additionally, several common functions from {dplyr} can be applied to survey objects including `filter()`, `mutate()`, and `summarize()`. 
