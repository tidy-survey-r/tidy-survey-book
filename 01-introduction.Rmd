...
The {srvyr} package in R builds on the {survey} package. It provides wrappers for functions that align with the tidyverse philosophy, which is our motivation for using and recommending this package. We find that the {srvyr} package is user-friendly for those familiar with tidyverse packages in R [@R-tidyverse]. For example, while many functions in the {survey} package use variables as formulas, the {srvyr} package uses [tidy selection](https://dplyr.tidyverse.org/reference/dplyr_tidy_select.html) to pass variable names (a common feature in the tidyverse). Users of the tidyverse are most likely familiar with the magrittr pipe (`%>%`), which seamlessly works with functions from the {srvyr} package. Moreover, several common functions from {dplyr}, such as including `filter()`, `mutate()`, and `summarize()`, can be applied to survey objects [@R-dplyr]. Users can streamline their analysis workflow and capitalize on the benefits of both the {srvyr} and {tidyverse} packages.
