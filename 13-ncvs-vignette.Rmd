...
For example, we could be interested in the percentage of property victimizations reported to the police as shown below with an estimate, the standard error, and 95% confidence interval:
...
The output from all the estimates is cleanded to create better labels such as going from "RaceHispOrigin" to "Race/Hispanic Origin". Finally, the {gt} package is used to make a publishable table shown in Table \@ref(tab:ncvs-vign-rates-demo-tab). Using the functions from the {gt} package, column labels and footnotes are added and estimates are presented to the first decimal place.
...
4. What is the difference between the violent victimization rate between males and females? Is it statisticallly different?
