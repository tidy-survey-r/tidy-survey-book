...
ICPSR distributes five files with unique identifiers:
...
We will focus on the household, person, and incident files. From these files, we selected a subset of columns to use in this vignette. Find the subsetted data in the book's GitHub repository.

## Data file preparation

Some work is necessary to prepare the files before analysis. The design variables indicating pseudostratum (`V2117`) and half-sample code (`V2118`) are only included on the household file so need to be added to the person and incident files for any analysis. 

To calculate victimization rates, we need to know about the victimization status for both victims and non-victims so the incident file must be summarized and then merged on to the household or person files for person-level and household-level crimes, respectively. We begin this vignette discussing how to create these incident summary files. This is following Section 2.2 of the NCVS User's Guide.
...
hh_vsum_der <- hh_vsum %>%
  mutate(
    Tenure = factor(
      case_when(str_detect(V2015, "1") ~ "Owned",
                !is.na(V2015) ~ "Rented"),
    ...
    PlaceSize = case_when(V2126B_num <= 20 ~ str_sub(V2126B, 6),
