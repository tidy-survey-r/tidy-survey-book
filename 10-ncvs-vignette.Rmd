...
We will focus on the household, person, and incident files. From these files, we selected a subset of columns for examples and have included the subset on the book's GitHub repository and the file subsets will be used in this vignette. Find the dataset in the book's GitHub repository or download the full files at ICPSR^[https://www.icpsr.umich.edu/web/NACJD/studies/38429].
...
inc_in <- get_file_osf_tsr("ncvs_2021_incident.rds")
...
In the code above, some checking of derived variables is done. This is a good point and pause to look at the output of crosswalks between an original variable and a derived to check the logic was programmed correctly and everything ends up in the bucket you expect. 
...
hh_in <- get_file_osf_tsr("ncvs_2021_household.rds")

pers_in <- get_file_osf_tsr("ncvs_2021_person.rds")
...
NHOPI <- "Native Hawaiian or Other Pacific Islander" # made for usage later
