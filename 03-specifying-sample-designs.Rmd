...
$$se(\hat{p}) = \frac{1}{N} \sqrt{ \sum_{h=1}^H N_h^2 \frac{\hat{p}_h(1-\hat{p}_h)}{n_h} \left(1-\frac{n_h}{N_h}\right)}$$
...
Consider a population where there are $A$ clusters and $a$ clusters are sampled via SRS. Units within each sampled cluster are sampled via SRS as well. Let $B_i$ be the number of units in cluster $i$ and $\bar{y}_i$ be the sample mean of cluster $i$. Then, a ratio estimator of the population mean is:

$$\bar{y}=\frac{\sum_{i=1}^a B_i \bar{y}_i}{ \sum_{i=1}^a B_i}$$
...
$$se(\bar{y})=\sqrt{\left(1-\frac{a}{A}\right)\frac{s_a^2}{a} + \frac{a}{A}\left(1-\frac{b}{B}\right) \frac{s_b^2}{ab}}$$
where $s_a^2$ is the between-cluster variance: 

$$s_a^2=\frac{1}{(a-1)b^2}\left(\sum_{\alpha=1}^a y_\alpha^2 - \frac{y^2}{a}\right)$$

and $s_b^2$ is the within-cluster variance:

$$s_b^2=\frac{1}{a(b-1)} \sum_{\alpha=1}^a \sum_{\beta=1}^b \left(y_{\alpha\beta}-\frac{y^2}{a}\right)$$
...
A weighted estimate for the full sample is calculated as $\hat{\theta}$, and then a weighted estimate for each replicate is calculated as $\hat{\theta}_r$ for $R$ replicates. Using the generic notation above, $\alpha=\frac{1}{R}$ and $\alpha_r=1 \forall r$. The standard error of the estimate is calculated as follows:
