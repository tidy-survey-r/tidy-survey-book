...
Stata, SPSS, and SAS files often contain labeled values. We can use the `haven::labeled()` function to import labeled numeric or character vectors and create an *intermediate* object that we convert into a regular R data frame. That is, we should not intend to keep the labeled vectors indefinitely. 
...
```{r}
#| eval = TRUE
response <- c("strongly agree", "strongly agree", "agree", "disagree")
response_levels <- c("strongly agree", "agree", "disagree", "strongly disagree")
factors <- factor(response, levels = response_levels)
factors
```

Recall that factors are integer vectors. We can confirm by looking at their structure:
...
Though factors differ from labeled vectors from Stata, SPSS, or SAS, we can leverage them to assign a dataset's labels with numeric or character vectors with `as_factor()`.

```{r}
as_factor(dta$V201039z)
```
