...
Survey files come in different file types depending on the program used to output them. R gives us the flexibility to load datasets regardless of their file extension. We will show how to load common public-use survey file formats:
...
We load our file into R by running `read_dat()`:

```{r}
#| eval = FALSE
library(haven)
read_dta("https://www.stata-press.com/data/r17/smho.dta")
```
...
We can use the `haven::labelled()` function to import labeled vectors into R. The goal is to create an intermediate object and then convert it into a regular R data frame to use in our analysis.
