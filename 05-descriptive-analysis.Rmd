...
* `vartype`: type(s) of variation estimate to calculate, defaults to `se` (standard error) (see \@ref(desc-count-syntax) for more information)
...
recs_des %>%
  summarize(DOLLARLP_Tot=survey_total(DOLLARLP),
...
It is estimated that American residential households spent an average of `r .elbill %>% pull(elec_bill)` on electricity in 2015 and the estimate has a standard error of `r .elbill %>% pull(elec_bill_se)`. The unweighted function calculates unweighted average and illustrates the average amount of money the *respondents* spent on electricity in 2015 which was `r .elbill_mn_unwgt %>% pull(elec_unweight)`. 
