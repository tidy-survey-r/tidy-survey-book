...
<!--<JM> Previous chapter had targetpop calculated from data? Can that be used here?-->
```{r}
#| label: desc-anes-des
#| eval: FALSE
...
<!--<JM> Dispersion is e.g. standard deviation? (right now says e.g. standard)-->
...
Thus, the estimated number of households in the U.S. is `r sub("\\..*", "", .est_pop)`. 
...
<!--<JM> xlogit requires user to have SUDAAN or SPSS installed? Not sure what is meant by it is implemented in SUDAAN and SPSS-->
...
When specifying multiple variables, the proportions are conditional. In the results above, notice that the proportions sum to 1 within each region. This can be interpreted as the proportion of housing units with air conditioning 
*within* each region. 
...
<!--<JM> Not sure what "lower performance" means-->
...
## Ratios
<!--<JM> Why is it necessary to say many are not familiar with the ratio estimate?  Also, why switch to the police example when you are on a roll with energy and there is no output with the police example?-->
...
<!--<JM> Say more about why you'd want to find a ratio for this example-->
...
Though not a statistical test, it does appear the cost ratios in the Midwest for liquid propane are the lowest.
