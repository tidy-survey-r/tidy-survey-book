...
To incorporate each of these survey functions, recall the general process for survey estimation from Chapter \@ref(c04-set-up):  
...
For the sake of simplicity, we've removed cases with missing values in the example below. If you want a more detailed explanation on how to handle missing data, please refer to Chapter \@ref(c11-missing-data).
...
The syntax for `survey_count()` is similar to the `dplyr::count()` syntax. However, as noted this function can only be called on `tbl_svy` objects. Let's explore the syntax: 
...
If we want to achieve the same result using `survey_tally()`, using the same syntax as `survey_count()` will result in an error:
...
Each option will yield slightly different confidence interval bounds when dealing with proportions. Please note that when working with `survey_mean()`, we do not need to specify this method unless the `proprtion` argument is `TRUE`.
...
One note with the `score` method is that when there are many ties in the data, this method can produce confidence intervals that do not contain the estimate. When dealing with a high propensity for ties (e.g., many respondents have the same age), it is recommended to use another method. This is the method implemented in SUDAAN. However, SUDAAN adds noise to the values to prevent the issue with the ties, while the documentation in the {survey} package indicates this method generally has poorer performance compared to the beta and logit intervals. Therefore, we recommend using these alternative methods instead.
...
First, let's look at an example calculating average household electricity cost and then build on it to examine the features of the `cascade()` function. In the first example, we haven't changed any of the argument defaults. We are calculating the average household energy cost `DOLLAREL_mn` using the `survey_mean()` function.
...
Next, we can group the results by region by adding `group_by(Region)` before the `cascade()` function.
...
The last row where `Region = NA` is the national average electricity bill. However, calling the region `NA` is not very informative, and we may wish to assign it a better name. We can do this using the `.fill` argument.
...
Though the results haven't changed, the table is easier to understand. The average national household electricity bill is $1380, and the other regions are listed below the first row.
