...
We will discuss `vartype` in a later section as this option occurs in all functions. We will introduce all the types of variance estimates in that section.
...
The `survey_tally()` function is similar to the `survey_count()` function with the syntax below:
...
The `survey_tally()` function lacks the following arguments `...` (used to specify the by groups) and `.drop`. To get the estimated overall population, `survey_count()` and `survey_total()` will seem identical and the example below yields the same results as using `survey_count()` previously.
...
The difference between `survey_total()` and `survey_count()` is more evident when specifying continuous variables to sum. Let's compute the total cost of electricity in whole dollars from variable `DOLLAREL`. We also calculate an unweighted estimate using `unweighted()`. The `unweighted()` function calculates unweighted summaries from `tbl_svy` object which reflects the summary among the **respondents** and does not extrapolate to a population estimate.
