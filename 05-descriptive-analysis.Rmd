...
The syntax for `survey_count()` is similar to the `dplyr::count()` syntax. However, as noted above, this function can only be called on `tbl_svy` objects. Let's explore the syntax: 
...
Each option will yield slightly different confidence interval bounds when dealing with proportions. Please note that when working with `survey_mean()`, we do not need to specify this method unless the `proportion` argument is `TRUE`.
...
The 25th percentile for the Northeast region is `r scales::dollar(recs_des %>% group_by(Region) %>% summarize(elec_bill = survey_quantile(DOLLAREL,  quantiles = c(0.25, .5, 0.75))) %>% filter(Region == "Northeast") %>% pull(elec_bill_q25))` while it is `r scales::dollar(recs_des %>% group_by(Region) %>% summarize(elec_bill = survey_quantile(DOLLAREL,  quantiles = c(0.25, .5, 0.75))) %>% filter(Region == "South") %>% pull(elec_bill_q25))` for the South.

#### Example 3: Minimum and maximum {.unnumbered}

As mentioned in the syntax section, specifying quantiles of `0` (minimum) and `1` (maximum). R will calculate these values.  However, these are only the minimum and maximum values in the data, and there is not enough information to determine their standard errors: 
...
The minimum cost of electricity in the dataset is `r recs_des %>% summarize(elec_bill = survey_quantile(DOLLAREL, quantiles = c(0, 1))) %>%  pull(elec_bill_q00)` while the maximum is
`r recs_des %>% summarize(elec_bill = survey_quantile(DOLLAREL, quantiles = c(0, 1))) %>%  pull(elec_bill_q100)`, but the standard error is shown as `NaN` and 0, respectively. Notice that the minimum cost is a negative number. We should investigate the value to gain a better understanding of its meaning.
...
Nationally, the median household spent `r pull(.elbill_med, elec_bill)` in 2020. This is the same result as we obtained using the `survey_quantile()` function (`r pull(.elbill_quant, elec_bill_q50)`). Interestingly, the average electric bill for households that we calculated in section \@ref(desc-meanprop) is `r pull(.elbill_mn, elec_bill)`, but the estimated median electric bill is `r pull(.elbill_med, elec_bill)` indicating the distribution is likely right-skewed.
...
Although not a formal statistical test, it appears that the cost ratios for liquid propane are the lowest in the Midwest (`r round(recs_des %>% group_by(Region) %>% summarize(DOL_BTU_Rat = survey_ratio(DOLLARLP, BTULP)) %>% filter(Region == "Midwest") %>% pull(DOL_BTU_Rat), 2)`).
...
The correlation between total square footage of homes and electricity consumption is `r recs_des %>% summarize(SQFT_Elec_Corr = survey_corr(TOTSQFT_EN, BTUEL)) %>% pull(SQFT_Elec_Corr)`, indicating a moderate positive relationship.
...
For homes without air conditioning, there is a moderate positive correlation between total square footage with electricity consumption (`r recs_des %>% group_by(ACUsed) %>% summarize(SQFT_Elec_Corr = survey_corr(TOTSQFT_EN, DOLLAREL)) %>% filter(ACUsed == FALSE) %>% pull(SQFT_Elec_Corr)`). For homes with air conditioning, the correlation of `r recs_des %>% group_by(ACUsed) %>% summarize(SQFT_Elec_Corr = survey_corr(TOTSQFT_EN, DOLLAREL)) %>% filter(ACUsed == TRUE) %>% pull(SQFT_Elec_Corr)` indicates a stronger positive correlation betwen total square footage and electricity consumption.
...
The estimated average amount spent on natural gas is `r scales::dollar(recs_des %>% filter(BTUNG > 0) %>% summarize(NG_mean = survey_mean(DOLLARNG, vartype = c("se", "ci"))) %>% pull(NG_mean))`.
...
Based on this calculation, the estimated average amount spent on natural gas is `r scales::dollar(recs_des %>% summarize(NG_mean = survey_mean(DOLLARNG, vartype = c("se", "ci"))) %>% pull(NG_mean))`.
...
We can see the estimated average electricity bills by regions: `r scales::dollar(recs_des %>% group_by(Region) %>% cascade(DOLLAREL_mn = survey_mean(DOLLAREL)) %>% filter(Region == "Northeast") %>% pull(DOLLAREL_mn))` for the Northeast, `r scales::dollar(recs_des %>% group_by(Region) %>% cascade(DOLLAREL_mn = survey_mean(DOLLAREL)) %>% filter(Region == "South") %>% pull(DOLLAREL_mn))` for the South, and so on. The last row where `Region = NA` is the national average electricity bill, `r scales::dollar(recs_des %>% group_by(Region) %>% cascade(DOLLAREL_mn = survey_mean(DOLLAREL)) %>% filter(is.na(Region)) %>% pull(DOLLAREL_mn))`. However, naming the national "region" as `NA` is not very informative. We can give it a better name using the `.fill` argument.
...
Suppose we want to calculate the total and average consumption, along with coefficients of variation (CV), for each fuel type: the reported consumption of electricity (`BTUEL`), natural gas (`BTUNG`), liquid propane (`BTULP`), fuel oil (`BTUFO`), and wood (`BTUWOOD`), as mentioned in the section on design effects. We can take advantage of the fact that these are the only variables that start with "BTU" by selecting them with `starts_with("BTU")` in the `across()` function. For each selected column (`.x`), `across()` creates a list of two functions to be applied: `survey_total()` to calculate the total and `survey_mean()` to calculate the mean, along with their CV (`vartype = "cv"`). Finally, `.unpack = "{outer}.{inner}"` specifies that the resulting column names are a concatenation of the variable name, followed by Total or Mean, and then "coeff" or "cv".
...
The estimated total consumption of electricity (`BTUEL`) is `r scales::comma(consumption_ests %>% pull(BTUEL_Total.coef))` (`BTUEL_Total.coef`), the estimated average consumption is `r scales::comma(consumption_ests %>% pull(BTUEL_Mean.coef))` (`BTUEL_Mean.coef`), and the CV is `r round(consumption_ests %>% pull(BTUEL_Total._cv), 3)`.

In the example above, the table was quite wide. We may prefer a row for each fuel type. Using the `pivot_longer()` and `pivot_wider()` functions from {tidyr} can help us achieve this. We first elongate the data with `pivot_longer()` and separate the components of the name with `names_pattern`:
...
Then, we create a function to replace `TrustGovernment` with the argument provided to the function. This involves a bit of tidy evaluation, which is a more advanced skill. To learn more, we recommend @wickham2019advanced. 
...
We can then apply this function to the two variables of interest:
...
Finally, we can use `map()` to iterate over as many variables as needed. The output will be a tibble with the variable name in the "Variable" column, the responses in the "Answer" column, along with the percentage and standard error. This example extends nicely when dealing with numerous variables for which we want percentage estimates.
