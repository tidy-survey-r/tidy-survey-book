...
Now, we want to use the original series of steps as a template to create a general function `calcps()` that can apply the same series of steps to other variables. We replace `TrustGovernment` with an argument for the a generic variable, `var`. Referring to `var` involves a bit of tidy evaluation, an advanced skill. To learn more, we recommend @wickham2019advanced. 
...
We can then apply this function to the two variables of interest, `TrustGovernment` and `TrustPeople`:
...
Finally, we can use `map()` to iterate over as many variables as needed. The input is our desired variables. Within `map()`, we input our custom function `calcps`. The output will be a tibble with the variable name in the "Variable" column, the responses in the "Answer" column, along with the percentage and standard error. The `list_rbind()` function binds all the rows together and outputs them in a single tibble. This example extends nicely when dealing with numerous variables for which we want percentage estimates.
