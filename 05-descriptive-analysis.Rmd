...
The minimum cost of electricity in the dataset is `r recs_des %>% summarize(elec_bill = survey_quantile(DOLLAREL, quantiles = c(0, 1))) %>%  pull(elec_bill_q00)` while the maximum is `r recs_des %>% summarize(elec_bill = survey_quantile(DOLLAREL, quantiles = c(0, 1))) %>%  pull(elec_bill_q100)`, but the standard error is shown as `NaN` and 0, respectively. Notice that the minimum cost is a negative number. We should investigate the value to gain a better understanding of its meaning.
