...
Several functions in {srvyr} must be called within {srvyr}'s `summarize()` function. Recall that `dplyr::summarize()` collapses many values down to a single summary:
...
```{r}
mtcars %>% 
  dplyr::summarize(mean_hp = mean(hp), by = "cyl")
```
...
This practice can create a false impression of the distribution and lead to incorrect inferences because categorical variables lack a natural zero point or linear ordering, making measures like mean inappropriate. Instead, it is crucial to inspect the codebook, understand the type of variable, and choose appropriate measures such as frequency counts or percentages to describe the responses across regions.
...
When we run the cross-tab, we see that there are 5,638,844 responses from the Northeast's New England division.
...
Note that `recs_des %>% summarize(survey_total(), .groups = "drop")` is equivalent to the `survey_count()` call:
...
This is something to keep in mind (if we intend `proportion = FALSE)`.
...
`proportion = TRUE` is the default for `svy_prop()`. It makes confidence intervals more accurate near 0 and 1. Here, we rerun the previous function using the logit proportion method and with `proportion = FALSE`.
...
While we will not cover each one in depth; however, the same principles that we covered above apply.
