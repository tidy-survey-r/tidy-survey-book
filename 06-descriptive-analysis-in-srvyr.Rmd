...
Bivariate analysis concerns the relationship between two variables, while Multivariate analysis studies the relationship between multiple variables. The correlation is a number between -1 and +1 that denotes the strength of the linear relationship between two variables.

* Examples: Relationship between academic performance scores in 1999 and 2000; relationship between age and income 

The {srvyr} package does not include a function for calculating correlations for survey variables. We can use the {survey} package's `svyvar()` estimates the variance between the variables and then `cov2cor()` to get the correlation. Another option is to use the `svycor()` function from the {jtools} package.

```{r}
# Calculate correlation between API 2000 and API 1999.
...
dstrata |>
  ...
  (\(x) as.matrix(x))() |>
  mutate(across(everything(), var, NULL)) |>
  (\(x) cov2cor(x))()
```
...
We should use different measures depending on whether our variables are interval/ratio, ordinal, or nominal.
