...
where $y_i$ is the outcome, $\beta_0$ is an intercept, $x_1, \cdots, x_n$ are the predictors with $\beta_1, \cdots, \beta_n$ as the associated coefficients, and $\epsilon_i$ is the error. 
...
$$ \pi_i=\frac{\exp \left(\beta_0 +\sum_{i=1}^n \beta_i x_i \right)}{1+\exp \left(\beta_0 +\sum_{i=1}^n \beta_i x_i \right)}.$$
...
In the following example, the ANES data is used and we are modeling whether someone has trust usually in the government by who someone voted in 2020. As a reminder, the main candidates were Biden and Trump though people could vote for someone else in neither the Democratic or Republican parties. Those votes are all grouped into an "Other" category.
...
anes_des_der %>%
  ...
  summarise(
    pct_trust=survey_mean(TrustGovernmentUsually, na.rm=TRUE)
  )

logistic_out <- anes_des_der %>%
  svyglm(design=.,
         formula=TrustGovernmentUsually~ VotedPres2020_selection,
         family = quasibinomial) 

tidy(logistic_out)
tidy(logistic_out, exponentiate = TRUE) %>% select(term, estimate)
```

```{r}
#| label: logisticcalc
#| echo: false
or_trump <- tidy(logistic_out, exponentiate = TRUE) %>% filter(str_detect(term, "Trump")) %>% pull(estimate)
or_other <- tidy(logistic_out, exponentiate = TRUE) %>% filter(str_detect(term, "Other")) %>% pull(estimate)
