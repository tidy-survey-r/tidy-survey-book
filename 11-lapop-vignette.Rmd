...
This survey was administered in 2021 between March and August, varying by country^[See Table 2 in https://www.vanderbilt.edu/lapop/ab2021/AB2021-Technical-Report-v1.0-FINAL-eng-030722.pdf for dates by country]. Given the state of the pandemic at that time, several questions about COVID were included. The first question about COVID asked whether people were worried about the possibility that they or someone in their household will get sick from coronavirus in the next 3 months. We will calculate the percent of people in each country who are very worried or somewhat worried. 


In the following code, estimates are calculated and then using the {{gt}} package, a table of the estimates is created.
...
covid_worry_country_ests <-
  ...
  group_by(Country) %>%
  summarise(
    p=survey_mean(CovidWorry_bin=="WorriedHi", na.rm=TRUE)*100
  ) 
...
>  - Yes, they switched to a combination of virutal and in-person classes
>  - Yes, they cut all ties with the school

Multiple choice questions are interesting. If we want to look at how education was impacted only among those that were in school, we need to filter to the relevant responses which is anyone that responded no to the first part. An unweighted cross-tab fo the responses is included below and we can see there is a wide-range of impacts and that many combinations of impact on education are possible.

```{r}
#| label: ambarom-covid-ed-skip

ambarom %>% filter(Educ_NotInSchool==0) %>% count(Educ_NormalSchool, Educ_VirtualSchool, Educ_Hybrid, Educ_NoSchool) %>% print(n=50)
```
...
- Indicator that education medium was changed - either virtual or hybrid
...
ambarom_des_educ <- ambarom_des %>%
  ...
  mutate(
    Educ_OnlyNormal=Educ_NormalSchool==1 & Educ_VirtualSchool==0 & Educ_Hybrid==0,
...
Of the countries that used this data, many had households where their children had an education medium change with the exception of Haiti where only 7.2% of households that had students changed to virtual or hybrid learning. 
...
To merge on the data and make a map, we begin with the map file, merge on the estimates data, and then plot as shown below for the outcomes that we have used above in tables.
...
ggplot() +
  ...
  geom_sf_pattern(data=filter(covid_sf, is.na(p)), pattern="crosshatch", pattern_fill="black", fill=NA)
```
...
ggplot() +
  ...
  scale_fill_gradientn(guide="colourbar",name="Percent", labels=scales::comma, 
                       colours=c("#BFD7EA", "#087E8B", "#0B3954"), na.value=NA) +
  geom_sf_pattern(data=filter(covid_sf, is.na(p_mediumchange)), pattern="crosshatch", pattern_fill="black", fill=NA)
```

Canada, Mexico, and the United States did not include this question so it may make sense to remove North America from the map to focus on Central and South America. This is done below by restricting to Latin America and the Caribbean.
...
ggplot() +
  ...
  scale_fill_gradientn(guide="colourbar",name="Percent", labels=scales::comma, 
                       colours=c("#BFD7EA", "#087E8B", "#0B3954"), na.value=NA) +
  geom_sf_pattern(data=filter(covid_c_s, is.na(p_mediumchange)), pattern="crosshatch", pattern_fill="black", fill=NA)
```


## Exercises

1. Calculate the percentage of households with broadband internet and those with any internet in their home, including from phone or tablet. Hint: if you're seeing countries with 0% itnernet usage, you may want to filter by something first.
...
b_int_sf %>%
  ...
  scale_fill_gradientn(guide="colourbar",name="Percent", labels=scales::comma, 
                       colours=c("#BFD7EA", "#087E8B", "#0B3954"), na.value=NA) +
  geom_sf_pattern(data=filter(b_int_sf, is.na(p)), pattern="crosshatch", pattern_fill="black", fill=NA)
